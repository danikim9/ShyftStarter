...
export function ReminderComposer() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-[11px] font-semibold text-white/40 tracking-wide mb-1.5">리마인더 내용</label>
        <input
          value={label}
      ...
      <div>
        <label className="block text-[11px] font-semibold text-white/40 tracking-wide mb-1.5">시각</label>
        <input
          type="time"
          value={time}
          onChange={(e) => setTime(e.target.value)}
          className="w-full rounded-xl bg-white/6 border border-white/10 px-3.5 py-3 text-sm text-white outline-none focus:border-brand-400/50"
        />
      </div>
      <PrimaryButton
        disabled={!label.trim()}
