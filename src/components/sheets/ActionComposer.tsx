    ...
    <div className="space-y-4">
      <div className="inline-flex items-center gap-0.5 rounded-full bg-ink-950/6 p-1">
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition ${mode === 'manual' ? 'bg-white text-ink-950' : 'text-ink-950/50'}`}
        >
          직접 만들기
        </button>
        <button
          onClick={() => setMode('ai')}
          className={`px-3 py-1.5 rounded-full text-xs font-semibold transition flex items-center gap-1 ${mode === 'ai' ? 'bg-white text-ink-950' : 'text-ink-950/50'}`}
        >
            ...
            <Card className="space-y-2.5">
              {suggestions.map((s, i) => (
                <div key={i} className="flex items-center justify-between gap-2">
                  <span className="text-sm text-ink-950/85">{s}</span>
                  <button
                    onClick={() => addAndClose(s)}
                    className="text-[11px] font-semibold px-2.5 py-1 rounded-lg bg-ink-950/10 text-ink-950/80 shrink-0"
                  >
                    추가
                  </button>
                </div>
              ))}
              <button onClick={closeSheet} className="text-[11px] text-ink-950/35 pt-1">
                모두 추가했어요, 닫기
              </button>
            </Card>
          )}
