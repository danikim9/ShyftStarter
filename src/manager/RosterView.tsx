...
function ShiftPill({ entry }: { entry: RosterEntry }) {
  if (entry === 'off') {
    return <span className="text-[11px] font-medium text-ink-950/30">휴무</span>
  }
  return (
    <span className="text-[11px] font-medium text-brand-700 tabular-nums">
...
export function EditShiftModal({
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${open ? '' : 'pointer-events-none'}`} aria-hidden={!open}>
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-ink-900 border border-ink-950/10 shadow-2xl">
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-ink-950/8">
          <div>
            <h3 className="text-sm font-semibold text-ink-950">근무 배정</h3>
            <p className="text-xs text-ink-950/40 mt-0.5">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          >
            <span className="text-sm text-ink-950/80">휴무로 표시</span>
            <span className={`w-9 h-5 rounded-full relative transition-colors ${isOff ? 'bg-brand-500' : 'bg-ink-950/15'}`}>
          ...
          <div className={`grid grid-cols-2 gap-3 transition-opacity ${isOff ? 'opacity-30 pointer-events-none' : ''}`}>
            <div>
              <label className="block text-[11px] font-semibold text-ink-950/40 tracking-wide mb-1.5">시작</label>
              <input
                type="time"
                value={start}
            ...
            <div>
              <label className="block text-[11px] font-semibold text-ink-950/40 tracking-wide mb-1.5">종료</label>
              <input
                type="time"
                value={end}
          ...
          {repeatTargetDate && onApplyToDate && (
            <label className="flex items-center gap-2.5 rounded-lg bg-ink-950/4 border border-ink-950/8 px-3 py-2.5 cursor-pointer">
              <input
                type="checkbox"
                checked={repeat}
              ...
              />
              <span className="text-xs text-ink-950/60">
        ...
        <div className="px-6 py-4 border-t border-ink-950/8 flex gap-2">
  ...
  return (
    <Card className="space-y-3 border-brand-400/20 bg-brand-500/[0.04]">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-ink-950">근무 교대 알림</h3>
      ...
      <p className="text-[11px] text-ink-950/35 leading-relaxed -mt-1">
      ...
      <div className="space-y-2">
        {recent.map((r) => {
          ...
          return (
            <div key={r.id} className="flex items-center justify-between gap-3 rounded-xl bg-ink-950/5 border border-ink-950/8 px-3.5 py-3">
              <div className="min-w-0">
                <div className="text-sm text-ink-950/85">
                  <span className="font-semibold">{r.requesterName}</span>
                  <span className="text-ink-950/40"> · {req.md}({req.dow}) 근무</span>
                  <ArrowLeftRight size={11} className="inline mx-1.5 text-ink-950/30" />
                  <span className="font-semibold">{r.targetMemberName}</span>
                  <span className="text-ink-950/40"> · {tgt.md}({tgt.dow}) 근무</span>
                </div>
                {r.note && <p className="text-xs text-ink-950/40 mt-1 leading-relaxed">"{r.note}"</p>}
              </div>
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-ink-950 mb-1">근무 일정 관리</h1>
          <p className="text-ink-950/40 text-sm max-w-xl leading-relaxed">
            팀원별 근무를 배정·변경하고 휴무를 한눈에 확인해요. 셀을 클릭하면 시간을 조정하거나 휴무로 바꿀 수 있어요.
          </p>
        </div>
        <div className="shrink-0 flex items-center gap-1 rounded-full bg-ink-800/90 border border-ink-950/10 p-1">
          <button
            onClick={() => setWeekIndex((i) => Math.max(0, i - 1))}
            disabled={weekIndex === 0}
            className="w-7 h-7 flex items-center justify-center rounded-full text-ink-950/50 hover:text-ink-950/80 disabled:opacity-25 disabled:hover:text-ink-950/50"
          >
            <ChevronLeft size={14} />
          </button>
          <span className="text-xs font-medium text-ink-950/80 px-2 min-w-[52px] text-center">{week.label}</span>
          <button
      ...
      <Card className="p-0 overflow-hidden">
        <div className="overflow-x-auto app-scroll">
          <table className="w-full border-collapse min-w-[760px]">
            <thead>
              <tr>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-ink-950/40 tracking-wide sticky left-0 bg-ink-800/95 z-10 w-36">
                  팀원
                </th>
                {week.dates.map((d) => {
                  ...
                  return (
                    <th
                      ...
                      className={`px-2 py-3 text-center text-[11px] font-semibold tracking-wide ${
                        isToday ? 'text-brand-700' : 'text-ink-950/40'
                      }`}
                    >
                      <div>{md}</div>
                      <div className={isToday ? 'text-brand-600' : 'text-ink-950/25'}>{dow}{isToday ? ' · 오늘' : ''}</div>
                    </th>
                  )
                })}
              </tr>
            ...
            <tbody>
              {ROSTER_MEMBERS.map((m) => (
                <tr key={m.id} className="border-t border-ink-950/6">
                  <td className="px-4 py-2.5 sticky left-0 bg-ink-800/95 z-10">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-ink-950/85 truncate">{m.name}</span>
                    </div>
                  </td>
                  {week.dates.map((d) => {
                    ...
                    return (
                      <td key={d} className={`px-1.5 py-2 text-center ${isToday ? 'bg-brand-500/[0.06]' : ''}`}>
                        <button
                          ...
                          className="w-full rounded-lg py-1.5 hover:bg-ink-950/6 transition"
                        >
            ...
            <tfoot>
              <tr className="border-t border-ink-950/8">
                <td className="px-4 py-2.5 sticky left-0 bg-ink-800/95 z-10">
                  <div className="flex items-center gap-1.5 text-[11px] text-ink-950/35">
                    <Users size={11} /> 근무 인원
                  </div>
                </td>
                {week.dates.map((d) => {
                  ...
                  return (
                    <td key={d} className="px-1.5 py-2.5 text-center text-[11px] font-semibold text-ink-950/50 tabular-nums">
                      {count}명
                    </td>
                  )
                })}
