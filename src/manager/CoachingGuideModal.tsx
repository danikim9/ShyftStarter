...
export function CoachingGuideModal() {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${open ? '' : 'pointer-events-none'}`} aria-hidden={!open}>
      ...
      <div
        className={`relative w-full max-w-lg max-h-[85vh] rounded-2xl bg-ink-900 border border-white/10 shadow-2xl flex flex-col transition-all duration-300 ${
          ...
          <>
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/8 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white">1:1 코칭 대화 가이드 — {member.name}</h3>
              </div>
              <button onClick={closeCoachingGuide} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/8 text-white/60">
                <X size={15} />
              </button>
            </div>
            ...
            <div className="overflow-y-auto app-scroll px-6 py-5 grow space-y-3">
              ...
              {steps.map((s, i) => (
                <div key={i} className="rounded-xl bg-white/5 border border-white/10 p-3.5">
                  <div className="text-[11px] font-semibold text-brand-300 mb-1">{s.step}</div>
                  <p className="text-sm text-white/80 leading-relaxed">{s.prompt}</p>
                </div>
              ))}
            </div>
