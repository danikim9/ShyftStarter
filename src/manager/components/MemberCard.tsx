...
export function MemberActionButton({ member, action }: { member: TeamMember; action: string }) {
  ...
  return (
    <button
      ...
      className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-ink-950/8 hover:bg-ink-950/14 text-ink-950/85 transition"
    >
      {ACTION_LABEL[action]}
    </button>
  )
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-ink-950 font-semibold text-sm">{member.name}</div>
            <div className="text-ink-950/40 text-xs">{member.role} · {member.tenure}</div>
          </div>
        </div>
        <QuadrantBadge quadrant={quadrant} />
      </div>
      <p className="text-sm text-ink-950/70 leading-relaxed mb-3">{member.signal}</p>
...
export function RosterRow({ member }: { member: TeamMember }) {
  ...
  return (
    <button
      onClick={() => openDetail(member.id)}
      className="w-full flex items-center gap-3 py-3 border-b border-ink-950/6 last:border-0 text-left"
    >
      <span
      ...
      <div className="min-w-0 flex-1">
        <div className="text-sm text-ink-950/90 font-medium">{member.name}</div>
        <div className="text-xs text-ink-950/40">{member.role} · {member.tenure}</div>
      </div>
      <div className="hidden sm:block text-xs text-ink-950/40 tabular-nums w-24 text-right">
