...
export function Coach() {
  ...
  return (
    <div className="px-4 pt-5 pb-8 space-y-5">
      <div>
        <h1 className="text-xl font-bold text-ink-950 mb-1">AI Coach</h1>
        <p className="text-xs text-ink-950/40">Your AI performance companion — 감시가 아닌, 성장을 위한 코치예요.</p>
      </div>

      <div className="flex items-center gap-2">
        <Badge tone="brand">{meta.nameKo}</Badge>
        <span className="text-[11px] text-ink-950/35">방금 생성됨</span>
      </div>

      <Card>
        <SectionLabel>WHAT HAPPENED?</SectionLabel>
        <p className="text-sm text-ink-950/85 leading-relaxed">{card.what}</p>
      </Card>

      <Card>
        <SectionLabel>WHY DOES IT MATTER?</SectionLabel>
        <p className="text-sm text-ink-950/85 leading-relaxed">{card.why}</p>
      </Card>

      <Card className="bg-brand-600/20 border-brand-400/30">
        <SectionLabel>WHAT SHOULD I DO NEXT?</SectionLabel>
        <p className="text-base text-ink-950 font-medium leading-snug">{card.nextAction.label}</p>
        <div className="mt-1 text-xs text-ink-950/50">예상 소요 시간 · {card.nextAction.durationMin}분</div>
      </Card>
      ...
      <div className="rounded-xl bg-ink-950/4 border border-ink-950/8 px-4 py-3 flex items-start gap-2.5">
