...
const SOURCE_META: Record<RecognitionSource, { label: string; icon: ReactNode; tone: string }> = {
  manager: { label: '매니저', icon: <Users size={12} />, tone: 'text-brand-600' },
  peer: { label: '동료', icon: <Users size={12} />, tone: 'text-emerald-600' },
  ai: { label: 'AI', icon: <Sparkles size={12} />, tone: 'text-amber-600' },
}
...
function LeaderboardToggle({ enabled, onChange }: { enabled: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-ink-950/6 text-[11px] font-medium text-ink-950/60"
    >
      <span className={`w-7 h-4 rounded-full relative transition ${enabled ? 'bg-brand-500' : 'bg-ink-950/15'}`}>
        <span
  ...
  return (
    <div className="px-4 pt-5 pb-8 space-y-6">
      <div>
        ...
        <p className="text-xs text-ink-950/40">강남점 동료들과 함께 성장해요 — 비교가 아니라 응원을 위한 공간이에요.</p>
      </div>
      ...
      <div>
        ...
        <Card className="bg-brand-600/15 border-brand-400/25">
          <div className="text-sm font-semibold text-ink-950 mb-1">{TEAM_CHALLENGE.title}</div>
          <p className="text-xs text-ink-950/60 leading-relaxed mb-3">{TEAM_CHALLENGE.description}</p>
          <div className="flex items-center justify-between text-xs text-ink-950/50 mb-1.5">
          ...
          />
          <div className="mt-3 pt-3 border-t border-ink-950/10 flex items-start gap-2">
            <span className="text-sm leading-none mt-0.5">🤖</span>
            <p className="text-xs text-ink-950/70 leading-relaxed">{challengeInsight}</p>
          </div>
          <div className="mt-2 text-[10px] text-ink-950/40">{TEAM_CHALLENGE.rewardNote}</div>
        </Card>
      </div>
            ...
            return (
              <div key={r.id} className="flex items-start gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-ink-950/35 mt-0.5">
                    {meta.label} · {r.date}
                  </div>
                </div>
              </div>
              ...
              >
                <span
                  className={`w-6 text-center text-xs font-bold shrink-0 ${
                    i === 0 ? 'text-amber-600' : i === 1 ? 'text-ink-950/70' : i === 2 ? 'text-orange-300' : 'text-ink-950/30'
                  }`}
                >
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm text-ink-950 font-medium truncate">
                    {entry.name} {entry.isMe && <span className="text-brand-600 text-[10px] font-semibold">나</span>}
                  </div>
                  <div className="text-[10px] text-ink-950/35">{entry.role}</div>
                </div>
                <div className="text-sm font-bold text-ink-950 tabular-nums">{entry.score}</div>
              </div>
            ))}
            <p className="text-[10px] text-ink-950/30 pt-2 mt-1 border-t border-ink-950/8 leading-relaxed">
              역량 종합 점수 기준 순위예요. 참여도·컨디션 등 민감한 지표는 리더보드에 노출되지 않아요.
            </p>
          </Card>
        ) : (
          <Card className="flex items-center gap-3 text-ink-950/40">
            <EyeOff size={18} />
