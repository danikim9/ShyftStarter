...
export function RoiAnalysis() {
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      <div>
        <h1 className="text-2xl font-bold text-ink-950 mb-1">KPI Correlation &amp; ROI</h1>
        <p className="text-ink-950/40 text-sm max-w-2xl leading-relaxed">
      ...
      <div>
        ...
        <Card className="bg-gradient-to-br from-brand-600/15 to-transparent border-brand-400/20">
          <div className="grid sm:grid-cols-3 gap-4 mb-4">
            <div>
              <div className="text-3xl font-bold text-ink-950 tabular-nums">r = {r.toFixed(2)}</div>
              <div className="text-[11px] text-ink-950/40 mt-0.5">이수율 ↔ ATV 상관계수</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-emerald-600 tabular-nums">+{uplift.upliftPct.toFixed(1)}%</div>
              <div className="text-[11px] text-ink-950/40 mt-0.5">
                이수율 상위 20% 매장({uplift.topCount}개)의 평균 ATV가 나머지 매장보다 높아요
              </div>
            </div>
            <div>
              <div className="text-3xl font-bold text-ink-950 tabular-nums">{fmtAtv(uplift.topAvg)}</div>
              <div className="text-[11px] text-ink-950/40 mt-0.5">상위 그룹 평균 ATV (나머지 {fmtAtv(uplift.restAvg)})</div>
            </div>
          </div>
          <CorrelationChart />
          <p className="text-xs text-ink-950/50 leading-relaxed mt-3 pt-3 border-t border-ink-950/10">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <Card>
            <p className="text-xs text-ink-950/50 mb-3">트레이닝(마이크로러닝+Role-play) 완료율 상위/하위 매장 그룹 비교</p>
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-xs text-ink-950/60 mb-1">
                  <span className="text-emerald-600 font-medium">상위 그룹 ({trainingSplit.top.length}개 매장)</span>
              ...
              <div>
                <div className="flex justify-between text-xs text-ink-950/60 mb-1">
                  <span className="text-ink-950/40 font-medium">하위 그룹 ({trainingSplit.bottom.length}개 매장)</span>
                  <span className="tabular-nums">ATV {fmtAtv(avgOf(trainingSplit.bottom, (s) => s.atv))} · CVR {avgOf(trainingSplit.bottom, (s) => s.cvr)}%</span>
                </div>
                <ProgressBar value={avgOf(trainingSplit.bottom, (s) => s.capabilityScore)} max={100} colorClass="bg-ink-950/25" />
              </div>
            </div>
          </Card>
        </div>
          ...
          <Card>
            <div className="flex items-start gap-2.5">
              ...
              <p className="text-xs text-ink-950/70 leading-relaxed">
                ...
                <span className="text-ink-950 font-semibold">r = {coachingR.toFixed(2)}</span>예요. 코칭 세션이
      ...
      <div>
        ...
        <Card>
          <p className="text-xs text-ink-950/45 mb-3 leading-relaxed">
          ...
          <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2">
            {BRAND_ALIGNMENT.sort((a, b) => b.score - a.score).map((b) => (
              <div key={b.storeId} className="flex items-center gap-2.5">
                <span className="text-xs text-ink-950/60 w-16 shrink-0">{b.storeName}</span>
                <div className="flex-1">
                  <ProgressBar value={b.score} max={100} colorClass="bg-brand-500" />
                </div>
                <span className="text-xs text-ink-950/50 tabular-nums w-8 text-right">{b.score}</span>
              </div>
            ))}
          </div>
        </Card>
